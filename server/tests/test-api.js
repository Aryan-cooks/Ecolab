...
async function runTests() {
  ...
  try {
    const res = await axios.post(`${BASE_URL}/footprint/calculate`, {
      inputs: testInputs,
    });
    calculationResults = res.data.results;
    if (
      calculationResults &&
      calculationResults.total !== undefined &&
      calculationResults.greenScore !== undefined &&
