...
export const useStore = create((set, get) => ({
  ...
  signup: async (email, password, displayName) => {
    ...
    if (!get().offlineMode) {
      ...
      set({ error: 'Cannot register in offline mode. Please connect to the server.', isLoading: false });
      return false;
    }
  ...
  login: async (email, password) => {
    ...
    if (!get().offlineMode) {
      ...
      set({ error: 'Cannot log in during offline mode. Please connect to the server.', isLoading: false });
      return false;
    }
  ...
  // Auth: Logout
  logout: () => {
    set({ user: null, footprint: null, suggestions: [], progress: [], actions: [] });
  ...
  updateProfile: async (profileUpdates) => {
    ...
    if (!get().offlineMode) {
      try {
        const res = await api.post('/users/profile', { uid: user.uid, profile: updatedUser });
        set({ user: res.data.user });
        localStorage.setItem('eco_user', JSON.stringify(res.data.user));
      } catch (err) {
  ...
  fetchSuggestions: async () => {
    ...
    if (!get().offlineMode) {
      try {
        ...
        console.error('API suggestions fetch failed, falling back to local rules-based recommendations.', err);
  ...
  addCustomSuggestion: async (suggestionData) => {
    ...
    if (!get().offlineMode) {
      try {
        await api.post('/suggestions/custom', { uid: user.uid, suggestion: newSuggestion });
      } catch (err) {
  ...
  completeAction: async (actionId, actionData) => {
    ...
    if (updatedScore > 800 && !badges.includes('carbon_neutralizer')) badges.push('carbon_neutralizer');
  ...
  sendChatMessage: async (messageText, history) => {
    ...
    const fp = footprint || JSON.parse(localStorage.getItem('eco_footprint')) || { total: 4200, breakdown: {} };
  ...
  completeWeeklyChallenge: async () => {
    ...
    if (!get().offlineMode) {
      try {
        await api.post('/users/profile', { uid: user.uid, profile: updatedUser });
      } catch (err) {
...
function loadLocalSuggestions(fp) {
  ...
  const breakdown = fp.breakdown || { transport: 1000, food: 1000, home: 1000, lifestyle: 500 };
  const primaryCategory = Object.entries(breakdown).sort((a,b)=>b[1]-a[1])[0]?.[0] || 'transport';

  const defaultSuggestions = [
    { id: 'mock_1', title: "Shift Commutes to Metro", description: "Use metro instead of petrol vehicle for your daily schedule.", category: "transport", savingKgPerYear: 480, difficulty: "Easy", timeToImplement: "This Week", status: "pending" },
    { id: 'mock_2', title: "1 Meatless Day Per Week", description: "Cut out high-methane meats for plant-based meals on Mondays.", category: "food", savingKgPerYear: 180, difficulty: "Easy", timeToImplement: "Immediate", status: "pending" },
    { id: 'mock_3', title: "Switch off Standby Devices", description: "Unplug chargers and appliances when idle to stop standby load draws.", category: "home", savingKgPerYear: 150, difficulty: "Easy", timeToImplement: "Immediate", status: "pending" },
    { id: 'mock_4', title: "Limit Wardrobe Size", description: "Skip fast fashion buys and wear high-quality items longer.", category: "lifestyle", savingKgPerYear: 340, difficulty: "Medium", timeToImplement: "This Month", status: "pending" },
    { id: 'mock_5', title: "Adopt Electric 2-Wheeler", description: "Swap out daily gasoline scooter trips for electric alternatives.", category: "transport", savingKgPerYear: 750, difficulty: "Medium", timeToImplement: "This Month", status: "pending" },
    { id: 'mock_6', title: "Source 80% Local Groceries", description: "Buy from local farmers and mandis to bypass heavy transport emissions.", category: "food", savingKgPerYear: 280, difficulty: "Medium", timeToImplement: "This Week", status: "pending" },
    { id: 'mock_7', title: "Install 5-Star BEE AC", description: "Upgrade old home air conditioning to new high-efficiency systems.", category: "home", savingKgPerYear: 820, difficulty: "Hard", timeToImplement: "This Month", status: "pending" },
    { id: 'mock_8', title: "Transition to Plant-Based Diet", description: "Commit fully to plant-based diet to avoid animal farming impact.", category: "food", savingKgPerYear: 650, difficulty: "Hard", timeToImplement: "This Month", status: "pending" }
  ];
...
function loadLocalLeaderboard() {
  const defaultLeaderboard = [
    { uid: 'leaderboard_priya', displayName: "Priya Nair", greenScore: 910, footprintTotal: 1200, level: "guardian", lastUpdated: "2026-06-11T12:00:00Z" },
    { uid: 'leaderboard_elena', displayName: "Elena Rostova", greenScore: 880, footprintTotal: 1800, level: "guardian", lastUpdated: "2026-06-11T12:00:00Z" },
    { uid: 'leaderboard_alex', displayName: "Alex Rivera", greenScore: 820, footprintTotal: 2400, level: "guardian", lastUpdated: "2026-06-11T12:00:00Z" },
    { uid: 'leaderboard_vikram', displayName: "Vikram Mehta", greenScore: 560, footprintTotal: 3800, level: "tree", lastUpdated: "2026-06-11T12:00:00Z" },
    { uid: 'leaderboard_samir', displayName: "Samir Sen", greenScore: 420, footprintTotal: 5100, level: "sapling", lastUpdated: "2026-06-11T12:00:00Z" }
  ];
