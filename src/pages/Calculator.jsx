...
export default function Calculator() {
  ...
  const inputs = {
    ...
    home: { monthlyKwh, lpgCylindersPerMonth: lpgCylinders, homeSizeSqm: homeSize },
    lifestyle: { monthlyClothingSpend: clothingSpend, screenHoursPerDay: screenHours, recyclingHabits: recycling }
  };
    ...
    <main className="flex-1 overflow-y-auto p-4 md:p-6 grid-line relative min-h-screen">
      ...
      <header className="flex justify-between items-end border-b border-outline pb-4 mb-6">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-primary tracking-tighter break-words">[SYSTEM.DATA_ANALYSIS_LAB]</h1>
          <p className="text-[10px] sm:text-xs text-secondary mt-1 opacity-80 uppercase tracking-widest break-words">Global Emissions Monitoring // Node: 0x244-EcoImpact</p>
        </div>
      ...
      <div className="grid grid-cols-12 gap-4 max-w-[1400px] mx-auto">
        ...
        {/* MODULE: TRANSPORT */}
        <section className="col-span-12 lg:col-span-6 border border-outline bg-surface p-4 relative">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-[10px] uppercase text-on-surface/60">Primary Vehicle Matrix</label>
              <select 
                value={vehicleType} 
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase text-on-surface/60">Kilo_Travel_Variable (Weekly)</label>
              <input 
                type="number" 
                value={weeklyKm} 
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase text-on-surface/60">Transit_Share_Ratio ({transitPercent}%)</label>
              <input 
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase text-on-surface/60">Aero_Flights (Annual)</label>
              <input 
                type="number" 
            ...
            <div className="col-span-2 mt-2">
              <div className="h-24 w-full border border-outline/50 bg-black/40 flex items-center justify-center relative overflow-hidden">
                <svg className="absolute opacity-20 w-full h-full" viewBox="0 0 200 100">
                  <path d="M0,50 Q50,0 100,50 T200,50" fill="none" stroke="#00FF41" strokeWidth="0.5"></path>
                  <path d="M0,60 Q50,10 100,60 T200,60" fill="none" stroke="#00FF41" strokeWidth="0.2"></path>
                  <circle cx="100" cy="50" fill="none" r="30" stroke="#00FF41" strokeDasharray="2,2" strokeWidth="0.5"></circle>
                </svg>
        ...
        <section className="col-span-12 lg:col-span-6 border border-outline bg-surface p-4 relative">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1 col-span-2 sm:col-span-1">
              <label className="text-[10px] uppercase text-on-surface/60">Protein_Source_Input</label>
              <select 
                value={dietType} 
            ...
            <div className="space-y-1 col-span-2 sm:col-span-1">
              <label className="text-[10px] uppercase text-on-surface/60">Waste_Residue_Delta</label>
              <select 
                value={wasteLevel} 
            ...
            <div className="space-y-1 col-span-2">
              <label className="text-[10px] uppercase text-on-surface/60">Local_Sourcing_Ratio ({localFoodPercent}%)</label>
              <input 
                type="range" min="0" max="100" 
            ...
            <div className="col-span-2 flex items-center gap-4 bg-black/40 p-2 border border-outline/30 mt-2">
              <div className="flex items-end gap-[2px] h-10">
                <div className="w-[3px] bg-primary animate-[bounce_1s_ease-in-out_infinite]" style={{animationDelay: '0.1s'}}></div>
                <div className="w-[3px] bg-primary animate-[bounce_1s_ease-in-out_infinite]" style={{animationDelay: '0.3s'}}></div>
                <div className="w-[3px] bg-primary animate-[bounce_1s_ease-in-out_infinite]" style={{animationDelay: '0.2s'}}></div>
                <div className="w-[3px] bg-primary animate-[bounce_1s_ease-in-out_infinite]" style={{animationDelay: '0.5s'}}></div>
                <div className="w-[3px] bg-primary animate-[bounce_1s_ease-in-out_infinite]" style={{animationDelay: '0.4s'}}></div>
              </div>
              <div className="text-[9px] text-secondary leading-tight uppercase">
                {wasteLevel === 'high' ? 'Warning: Organic waste exceeds threshold.' : 'Biological processing nominal.'}<br/>
        ...
        {/* MODULE: HOME & ENERGY */}
        <section className="col-span-12 lg:col-span-8 border border-outline bg-surface p-4 relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] uppercase text-on-surface/60">Power_Draw (kWh)</label>
                <input 
                  type="number" 
                  value={monthlyKwh} 
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase text-on-surface/60">LPG_Cylinders/Month</label>
                <input 
            ...
            <div className="col-span-1 sm:col-span-2 sm:border-l border-outline sm:pl-6 pt-4 sm:pt-0">
              <div className="flex justify-between items-center mb-2">
                <span className="text-[10px] uppercase text-on-surface/60">Thermal_Efficiency_Index</span>
                <span className="text-xs text-primary font-bold">{(Math.max(30, 100 - (monthlyKwh/10))).toFixed(1)}%</span>
              </div>
              <div className="w-full h-1 bg-outline rounded-full overflow-hidden">
                <div className="h-full bg-primary shadow-[0_0_8px_#00FF41] transition-all duration-500" style={{width: `${Math.max(30, 100 - (monthlyKwh/10))}%`}}></div>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-2">
                <div className="bg-black/40 border border-outline p-2">
                  ...
                  <div className="text-xs font-bold text-secondary">{(monthlyKwh * 0.6).toFixed(1)}kW/h</div>
                </div>
                <div className="bg-black/40 border border-outline p-2">
                  <div className="text-[8px] text-on-surface/50 uppercase">LUX_DEMAND_EST</div>
                  <div className="text-xs font-bold text-primary">{(monthlyKwh * 0.15).toFixed(1)}kW/h</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* MODULE: LIFESTYLE */}
        <section className="col-span-12 lg:col-span-4 border border-outline bg-surface p-4 relative">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <label className="text-[10px] uppercase text-on-surface/60">Material_Spend_Rate (INR)</label>
              <input 
                type="number" 
                value={clothingSpend} 
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase text-on-surface/60">Screen_Uptime_Cycles (Hours/Day)</label>
              <input 
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase text-on-surface/60">Recycle_Buffer_Status</label>
              <select 
                value={recycling} 
            ...
            <div className="pt-2 border-t border-outline/30">
              <p className="text-[9px] text-on-surface/70 leading-relaxed italic">
                {clothingSpend > 5000 ? '"System detected high frequency of material acquisition. Suggesting reduction."' : '"Material intake nominal."'}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="col-span-12 border-2 border-primary bg-black p-6 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-3 bg-primary text-background font-black text-[10px]">REAL_TIME_ANALYSIS</div>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="space-y-1">
              <div className="text-xs text-primary/60 uppercase font-bold tracking-widest">Total_Annual_CO2e_Delta</div>
              <div className="text-6xl md:text-7xl font-black text-primary leading-none tracking-tighter">
                {liveResults.totalTons.toFixed(1)}<span className="text-2xl ml-2 font-normal">t</span>
              </div>
            </div>
            <div className="flex flex-wrap md:flex-nowrap gap-6 md:gap-12 text-left md:text-right w-full md:w-auto mt-4 md:mt-0">
              ...
              <div className="flex-1 md:flex-none border-l border-outline pl-6">
                <div className="text-[10px] text-primary uppercase underline">EFFICIENCY_RANK</div>
                <div className="text-xl font-bold text-primary">#{liveResults.level.toUpperCase().slice(0,4)}-{Math.round(liveResults.percentileRank)}</div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 flex h-4 border border-outline bg-surface">
            <div className="h-full bg-primary transition-all duration-500" style={{width: `${tPct}%`}} title={`TRANSPORT: ${Math.round(tPct)}%`}></div>
            <div className="h-full bg-secondary transition-all duration-500" style={{width: `${fPct}%`}} title={`FOOD: ${Math.round(fPct)}%`}></div>
            <div className="h-full bg-accent-blue transition-all duration-500" style={{width: `${hPct}%`}} title={`HOME: ${Math.round(hPct)}%`}></div>
            <div className="h-full bg-on-surface/40 transition-all duration-500" style={{width: `${lPct}%`}} title={`LIFESTYLE: ${Math.round(lPct)}%`}></div>
          </div>
      ...
      <footer className="mt-8 flex flex-col sm:flex-row justify-between items-center border-t border-outline pt-4 gap-4">
        <div className="text-[10px] uppercase opacity-50 hidden sm:block">Encryption: AES-256-GCM</div>
        <button 
          onClick={handleSave} 
