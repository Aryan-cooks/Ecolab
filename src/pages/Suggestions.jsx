      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-neon-green/30 pb-4 gap-4">
          <div className="min-w-0">
            <h2 className="text-lg sm:text-xl font-bold text-neon-green terminal-glow break-words">
              [ NEUTRALIZATION_PROTOCOLS ]
            </h2>
            <p className="text-[10px] text-neon-amber mt-1">
              Manage Carbon Offset Tasks // Node-AR992
            </p>
          </div>
          <button
          ...
          <div className="border-2 border-neon-amber p-6 bg-surface animate-fade-in">
            ...
            <form onSubmit={handleManualLog} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-[9px] uppercase text-neon-green/60">
                    Estimated Savings (kg CO2e/year)
                  </label>
                  <input
                    type="number"
                    min="0"
              ...
              <div className="space-y-1">
                <label className="text-[9px] uppercase text-neon-green/60">
                  Description / Execution Notes (Optional)
                </label>
                <textarea
                  value={logNotes}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {pending.length > 0 ? (
                pending.map((item) => (
                  <div
                    key={item.id}
                    className={`border-2 p-5 bg-surface flex flex-col justify-between ${
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          ...
                          className={`border py-1.5 text-[9px] font-bold hover:bg-black transition-colors ${
                            item.difficulty === "Hard"
                              ? "border-neon-red text-neon-red hover:text-white"
                              : item.difficulty === "Medium"
                                ? "border-neon-amber text-neon-amber hover:text-white"
                                : "border-neon-green text-neon-green hover:text-white"
                          }`}
                        >
                          EXECUTE
                        </button>
            ...
            <div className="space-y-3">
              {completed.length > 0 ? (
                completed.map((item) => (
                  <div
                    key={item.actionId}
                    className="border border-neon-green/40 p-4 bg-surface flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
                  >
                    <div>
                    ...
                    <div className="text-right w-full sm:w-auto">
                      <div className="text-neon-amber text-xs font-bold">
                        -{item.savingKgPerYear} kg CO2e/yr
                      </div>
                      <div className="text-[8px] opacity-50 uppercase mt-0.5">
                        EST_ANNUAL_SAVINGS
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {dismissed.length > 0 ? (
                dismissed.map((item) => (
                  <div
                    key={item.id}
                    className="border border-neon-green/20 p-5 bg-surface flex flex-col justify-between opacity-55 hover:opacity-100 transition-opacity"
                  >
                    <div>
                      <div className="flex justify-between items-start mb-4">
